...
class JoystickEvent:
    def __init__(self, data):
        ts, value, type, number = struct.unpack('LhBB', data)

        self.time = ts
        self.type = type & 0x7f
        self.number = number
        self.init = (type & JS_EVENT_INIT) != 0

        if self.type == JS_EVENT_BUTTON:
            self.value = value
        else:
            self.value = float(value) / 0x7fff

    def __repr__(self):
        return '<JoystickEvent %s>' % repr(self.__dict__)
        

EVENT_SIZE = 8

def read_event(file):
    return JoystickEvent(file.read(EVENT_SIZE))
