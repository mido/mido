...
for i in range(4):
    ...
    track.append(Message('note_on', note=note, velocity=100, delta_ticks=delta))
    for j in range(delta // ticks_per_expr):
        pitch = MAX_PITCHWHEEL * j * ticks_per_expr // delta
        track.append(Message('pitchwheel', pitch=pitch, delta_ticks=ticks_per_expr))
    track.append(Message('note_off', note=note, velocity=100, delta_ticks=0))
