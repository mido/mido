...
import errors like this one::
    ...
    ImportError: No module named mido.backends.rtmidi

The reason is that Mido uses ``import_module()`` to import the backend
modules, while PyInstaller looks for ``import`` statements.

The example below uses the default :doc:`RtMidi backend <backends/rtmidi>`.
Install its dependency in the environment used to run PyInstaller::
...
The easiest fix is to import the module at the top of the program::
    ...
    import mido
    import mido.backends.rtmidi  # The backend you want to use.
