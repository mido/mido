...
def test_running_status_interruption():
    """Running status shall be reset by system common messages and
     invalid data bytes ignored."""
...
def test_running_status_common():
    """The last known status byte should not be reused with common or
     exclusive message"""
...
def test_sysex_inside_sysex():
    """Sysex inside sysex should end the first sysex message and
    start a new one."""
...
def test_stray_data_bytes():
    ...
    assert (tokenize([0, 1, 0x90, 2, 3, 0xf8, 6])
            == [[0x90, 2, 3], [0xf8]])
